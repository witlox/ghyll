...
Feature: Status state machine engine
  ...
  # See specs/architecture/components/state-machine.md.
  ...
  Scenario Outline: Invalid clause transition rejected (illegal-transition matrix)
    ...
    Then the engine rejects the transition with "illegal-transition: <from> -> <to> not in clause state machine"
  ...
  Scenario: Depth-below-required produces unevaluated
    ...
    Then the engine records status "unevaluated" with reason "depth-below-required"
  ...
  Scenario: Finding resolved by re-attack
    ...
    When the adversarial phase re-attacks after producer remediation and cannot reproduce F1
  ...
  Scenario: Producer cannot accept own risk
    ...
    And requires the verdict to come from the attestation flow component with operator op-id
  ...
  Scenario: Pass aborted by crash recovery
    ...
    Then the engine finds the orphaned pass and transitions it to "aborted" with reason "crash"
  ...
  Scenario: Residue computation
    ...
    And the project-status query reports R=75 alongside C and complete-against-grid-vN
  ...
  Scenario Outline: Illegal finding-status transitions rejected
    ...
    Then the engine rejects with "illegal-transition" and F1's status remains "<from>"
    ...
    Examples:
      ...
      # 17-20), those edges are LEGAL in v1:
      #   * resolved → open and accepted-risk → open are explicit
      #     operator-amendment paths (`TransitionWithReason` records
      #     the role + reason).
      #   * open → resolved is allowed when adversarial re-attack
      #     confirms a fix without an intermediate `running` state;
  ...
  Scenario: Crash while clause is awaiting-attestation
    ...
    Then crash recovery does NOT mark P1 as aborted (the operator can still deliver a verdict)
  ...
  Scenario: Residue with arithmetic overflow
    ...
    And R is reported with units operator-action-units, not a wrapped negative number
