...
Feature: Per-arrow adversarial phase
  ...
  Scenario: Arrow enters adversarial phase
    ...
    And R0 receives the upstream arrow artifact, the arrow's clause
        definitions, and the project's depth-ladder labels and
        per-requirement minimum depths
    And R0 runs the three sub-activities in order
    And the depth tier used by R0 meets the maximum depth-sensitivity
        requirement across the clauses
  ...
  Scenario: Adversary attempts to falsify each depth-sensitive clause
    ...
    And each successful falsification raises a finding with severity
        assigned per the stated rule

  Scenario: Falsification produces a finding
    Given clause C9 = "the negative space is specified" on an
        analyst→architect arrow
    When R0 finds that ContextA's spec mentions only happy paths with
        no rejection rules
    Then R0 raises finding F1 with type "clause-falsification",
        target-clause C9, severity "high", basis stating the rule and
        result, locations pointing at the relevant files, and evidence
  ...
  Scenario: Cannot falsify clause passes
    ...
    And R0 records its falsification attempt as "no defect found" in
        the adversarial-phase audit trail

  # ---- Open sweep sub-activity ----

  Scenario: Open sweep finds an un-clause-named defect
    Given the analyst's spec for ContextA references a service in
        ContextB that is never declared in
        cross-context/interactions.md
    When R0 sweeps
    Then R0 raises finding F2 with type "open-sweep", severity "high",
        basis "scope: cross-context references in features/contextA/,
        rule: every named external service must appear in
        cross-context/interactions.md; result: 1 missing", locations
        and evidence pointing at the missing declaration

  # ---- Depth classification sub-activity ----

  Scenario: Walk requirements on the depth ladder
    Given the upstream artifact contains a list of requirements with
        declared minimum depths
    When R0 enters depth classification
    Then R0 takes each requirement in turn
    And classifies it on the 4-tier depth ladder (default NONE /
        SHALLOW / MOCKED / REALISTIC; project overrides apply)
    And for any requirement classified below its declared minimum, R0
        raises a finding
  ...
  Scenario: Requirement below min
    ...
    Then R0 raises finding F3 with type "depth-below-min",
        target-requirement REQ-12, classified MOCKED, declared-min
        REALISTIC, severity "medium", basis "the requirement's tests
        all use mocks; no realistic-tier test against real dependency
        was found"
  ...
  Scenario: Adversary tier too shallow to classify
    ...
    And the arrow's verification will block via the auto-inserted
        "every-requirement-meets-min-depth" clause
  ...
  Scenario: Producer proposes accepted-risk
    ...
    When the producer proposes accepted-risk via a typed
        "accepted-risk-proposal" message containing pass-id,
        finding-id, rationale, and inspected-context
  ...
  Scenario: Non-convergence escalates after remediation-rounds-max
    Given finding F1 has been re-attacked through remediation-rounds-max
        rounds (default 5) and remains "open"
    ...
    And the operator must decide: accepted-risk OR route the artifact
        for deeper rework upstream

  # ---- Phase exit to verification ----

  Scenario: Convergence — all findings disposed
    Given all findings above the severity threshold are "resolved" or
        "accepted-risk"
