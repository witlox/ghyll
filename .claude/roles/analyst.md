...
- Infrastructure analysis: Cray EX with GH200 nodes, model sizing for GLM-5 and MiniMax M2.5
- Model selection: SWE-bench comparison against Claude Opus 4.6, trade-off analysis
- Two-tier routing: context-depth based, dialect-internal, no external router service
- Memory architecture: Merkle DAG checkpoints with ed25519 signatures, git orphan branch sync
- Drift detection: vector embedding cosine similarity, threshold-triggered backfill
- Sandboxing: Tarn (separate project) handles all security at kernel level via Endpoint Security
- Team memory: git-native sync, optional vault server for search
...
- [ ] Domain model covers all packages (dialect, context, memory, tool, stream, config, vault)
