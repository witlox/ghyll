...
- Prior art: see `docs/` for evaluation notes
- Seed terms: `specs/SEED.md` (if present)
...
- Test consistency: does the new requirement contradict existing invariants?
...
- [ ] Domain model covers all packages (dialect, context, memory, tool, stream, config, vault, workflow)
...
Write concrete Gherkin (specific values). Mark assumptions in `assumptions.md`.
Flag when a feature requires capabilities not yet specified.
