...
ghyll executes tools directly (YOLO mode by design). Blocking at the ghyll level would create a false sense of security and could be bypassed. A real sandbox — the operator chooses one from the [sandbox table](../operator-guide.md#sandbox-setup) — provides OS-level isolation that cannot be circumvented from user space, regardless of what ghyll executes. See [Why ghyll](../why.md#5-sandbox-only-execution) for the full rationale.
