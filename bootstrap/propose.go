...
//     the clause is recorded with merged args.
//   - Extend:  ExtensionClause must be non-nil; it is recorded as an
//     operator-added clause (no validation against the role
//     file — it is by definition not in the role file).
...
//   - ErrExtensionMissing if Kind==VerdictExtend and ExtensionClause==nil.
...
//     ErrModifyWeakening, ErrModifyNonFinite, etc.).
...
func (a *ArrowProposal) AllVerdictsReceived() bool {
	...
	return len(a.verdicts) == len(a.Proposed)
}

// cloneArgs returns a shallow copy of m. Returns nil for a nil input.
// Used so internal state cannot be mutated through the public Recorded
// slice and so the proposed clause's DefaultArgs is never shared with
// a Recorded entry.
...
// by any key present in diff. Inputs are not mutated.
